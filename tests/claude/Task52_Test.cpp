...
#include "../../source/output/claude/cpp/Task52_CLAUDE_claude-3-5-sonnet-20240620.cpp"
...
int main() {
    ...
    // اختبار 2: فك التشفير لملف غير موجود (نتوقع عدم إنشاء ملف مخرجات)
    try {
        decryptFile("non_existent_file.encrypted", decryptedFile, key);
        if (!fileExists(decryptedFile)) {
            cout << "Test 2 (decryptFile non-existent input): PASSED (no output file)\n";
            ++testsPassed;
        } else {
            cout << "Test 2 (decryptFile non-existent input): FAILED (output file created)\n";
            ++testsFailed;
        }
    } catch (...) {
        cout << "Test 2 (decryptFile non-existent input): PASSED (exception thrown)\n";
        ++testsPassed;
    }

    // اختبار 3: تشفير ملف غير موجود (نتوقع عدم إنشاء ملف مخرجات)
    try {
        encryptFile("non_existent_file.txt", encryptedFile, key);
        if (!fileExists(encryptedFile)) {
