...
#include <string>
#include <vector>
#include "../../../source/output/claude/cpp/Task183_CLAUDE_claude-3-5-sonnet-20240620.cpp"

void runTest(const std::string& input, int span, long long expected, int caseNumber, bool& allPassed) {
    long long actual = largestSeriesProduct(input, span);
    if (actual != expected) {
