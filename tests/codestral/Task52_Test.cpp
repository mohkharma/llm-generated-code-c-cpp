...
#include "../../source/output/codestral/cpp/Task52_MISTRAL_codestral-latest.cpp"  // Include the header for the function
...
int main() {
    ...
    // Define a fixed 128-bit key (16 bytes)
    ...
    try {
        ...
        if (!fileExists(decryptedFile)) {
            ...
            // We expect decryptedFile content to be same as testFile content (except maybe padding zeros)
            ...
            // Trim trailing zeros from decrypted content (padding)
    ...
    // Test 3: Encrypt non-existent file should not create output file
    try {
        encryptFile(key, "non_existent_input.txt", encryptedFile);
        if (!fileExists(encryptedFile)) {
    ...
    // Test 4: Decrypt non-existent file should not create output file
    try {
        decryptFile(key, "non_existent_input.encrypted", decryptedFile);
        if (!fileExists(decryptedFile)) {
