...
// --- دوال base64 encode/decode مدمجة ---
...
// دالة processRequest كما في كودك الأصلي
json processRequestWrapper(const std::string& request) {
    std::string raw_data = request;
    std::string decoded_data;
    try {
        decoded_data = base64_decode(raw_data);
    } catch (...) {
...
void runTests() {
    ...
    try {
        ...
        json result = processRequestWrapper(raw_data);
    ...
    try {
        std::string raw_data = "";
        processRequestWrapper(raw_data);
    ...
    try {
        ...
        processRequestWrapper(malformed);
