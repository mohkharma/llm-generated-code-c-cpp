...
void testBasicExample1() {
    ...
    assert(high_scoring_word("man i need a taxi up to ubud") == "taxi");
}

void testBasicExample2() {
    // Test basic example with different set of words
    assert(high_scoring_word("what time are we climbing up the volcano") == "volcano");
}

void testBasicExample3() {
    // Test basic example with a smaller set of words
    assert(high_scoring_word("take me to semynak") == "semynak");
}

void testSingleWord() {
    // Test with a single word
    assert(high_scoring_word("hello") == "hello");
}

void testMultipleWordsSameScore() {
    // Test with multiple words having the same score
    assert(high_scoring_word("apple banana cherry") == "cherry");
}

void testDifferentScores() {
    // Test with words having different scores
    assert(high_scoring_word("ant bear zebra") == "zebra");
}

void testWordsWithOneLetter() {
    // Test with single-letter words
    assert(high_scoring_word("a b c d e f g h i j k l m n o p q r s t u v w x y z") == "z");
}

void testEmptyString() {
    // Test with an empty string
    assert(high_scoring_word("") == "");
}
...
void testWordsWithSpecialCharacters() {
    ...
    assert(high_scoring_word("hello world") == "world");
}

void testLongWords() {
    // Test with long words to check performance
    assert(high_scoring_word("pneumoultramicroscopicsilicovolcanoconiosis supercalifragilisticexpialidocious antidisestablishmentarianism") == "pneumoultramicroscopicsilicovolcanoconiosis");
}
