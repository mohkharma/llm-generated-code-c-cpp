...
void testSingleElement() {
    ...
    assert(find_it(arr) == 7);
}

void testSingleZero() {
    std::vector<int> arr = {0};
    assert(find_it(arr) == 0);
}

void testOddOccurrence() {
    std::vector<int> arr = {1, 1, 2};
    assert(find_it(arr) == 2);
}
...
void testLargeArrayWithSingleOddOccurrence() {
    ...
    assert(find_it(arr) == 4);
}

void testNegativeNumbers() {
    std::vector<int> arr = {-1, -1, -2, -2, -3};
    assert(find_it(arr) == -3);
}

void testAllOddOccurrences() {
    std::vector<int> arr = {5, 5, 5, 7, 7, 7,7, 9, 9};//todo fix this case for all non cpp, except java is fixed
    // std::cout << find_it(arr) << std::endl;
