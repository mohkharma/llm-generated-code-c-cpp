...
#include "../../source/output/llama-3/cpp/Task39_PERPLEXITY_llama-3-sonar-large-32k-chat.cpp"  // Include the header for the function
int passed = 0;
int failed = 0;
...
void test_file_output_simple_text() {
    ...
    // إعادة التوجيه إلى ملف مؤقت
    std::string command = "./test_binary_runner > " + redirectOutputFile;  // placeholder for full app run
    // بدلًا من تشغيل كامل، نحاكي تنفيذ الأمر مباشرة
    std::string redirectCommand = "cat " + inputFilename + " > " + redirectOutputFile;
    std::system(redirectCommand.c_str());
