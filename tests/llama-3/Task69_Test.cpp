#include "../../source/output/llama-3/cpp/Task69_PERPLEXITY_llama-3-sonar-large-32k-chat.cpp"
...
void test_1_delete_existing_file() {
    ...
    deleteFileSimple(filename);

    std::cout.rdbuf(original);
    std::string output = buffer.str();

    ASSERT_TRUE(!std::filesystem::exists(filename) &&
                output.find("File deleted successfully") != std::string::npos,
...
void test_2_delete_non_existing_file() {
    ...
    ASSERT_TRUE(output.find("Failed to delete the file") != std::string::npos,
...
void test_3_permission_denied() {
    ...
    ASSERT_TRUE(true, "test_3_permission_denied");
}
...
void test_4_general_exception_handling() {
    ...
    ASSERT_TRUE(true, "test_4_general_exception_handling");
}
