...
void testSingleWord() {
    ...
    assert(highestScoringWord("hello") == "hello");
}

void testMultipleWordsSameScore() {
    // Test with multiple words having the same score
    assert(highestScoringWord("apple banana cherry") == "cherry");
}

void testDifferentScores() {
    // Test with words having different scores
    assert(highestScoringWord("ant bear zebra") == "zebra");
}

void testWordsWithOneLetter() {
    // Test with single-letter words
    assert(highestScoringWord("a b c d e f g h i j k l m n o p q r s t u v w x y z") == "z");
}

void testEmptyString() {
    // Test with an empty string
    assert(highestScoringWord("") == "");
}
...
void testWordsWithSpecialCharacters() {
    ...
    assert(highestScoringWord("hello world") == "world");
}

void testLongWords() {
    // Test with long words to check performance
    assert(highestScoringWord("pneumoultramicroscopicsilicovolcanoconiosis supercalifragilisticexpialidocious antidisestablishmentarianism") == "pneumoultramicroscopicsilicovolcanoconiosis");
}
