#include "../../source/output/gemini/cpp/Task69_GEMINI_gemini-1.5-pro-001.cpp"
...
void test_1_delete_existing_file() {
    ...
    deleteFileWithCheck(filename);

    std::cout.rdbuf(originalCout);
    std::string output = buffer.str();

    ASSERT_TRUE(!std::filesystem::exists(filename) &&
                output.find("deleted successfully") != std::string::npos,
...
void test_2_delete_non_existing_file() {
    ...
    std::stringstream buffer;
    std::streambuf* originalCerr = std::cerr.rdbuf(buffer.rdbuf());
    ...
    ASSERT_TRUE(output.find("not found") != std::string::npos,
                "test_2_delete_non_existing_file");
}

void test_3_permission_denied() {
    std::string filename = "/root/protected_file.txt";  // يتوقع أن يسبب خطأ صلاحيات
    ...
    ASSERT_TRUE(output.find("Error deleting file") != std::string::npos,
                "test_3_permission_denied");
}

void test_4_general_exception_handling() {
    std::string filename = "\0illegal_path";  // يفترض أن يسبب استثناء بسبب مسار غير صالح
