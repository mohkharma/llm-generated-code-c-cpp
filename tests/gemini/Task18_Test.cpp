...
void testbinary_array_to_number() {
    assert(binary_array_to_number({0, 0, 0, 1}) == 1); // 0001 = 1
    assert(binary_array_to_number({0, 0, 1, 0}) == 2); // 0010 = 2
    assert(binary_array_to_number({0, 1, 0, 1}) == 5); // 0101 = 5
    assert(binary_array_to_number({1, 0, 0, 1}) == 9); // 1001 = 9
    assert(binary_array_to_number({0, 1, 1, 0}) == 6); // 0110 = 6
    assert(binary_array_to_number({1, 1, 1, 1}) == 15); // 1111 = 15
    assert(binary_array_to_number({1, 0, 1, 1}) == 11); // 1011 = 11
    assert(binary_array_to_number({1}) == 1); // Single bit 1
    assert(binary_array_to_number({0}) == 0); // Single bit 0
    assert(binary_array_to_number({0, 1, 0, 1, 0, 1, 1}) == 43); // 0101011 = 43

    std::cout << "All test cases passed.\n";
}

int main() {
    testbinary_array_to_number();
    return 0;
}