...
vector<int> mergeSortedArrays(const vector<int>& arr1, const vector<int>& arr2) {
    // Sort input arrays to handle any input order (including reverse)
    ...
    int i = 0, j = 0;
    while (i < (int)sorted1.size() && j < (int)sorted2.size()) {
        if (sorted1[i] <= sorted2[j]) {
            mergedArray.push_back(sorted1[i++]);
        } else {
            mergedArray.push_back(sorted2[j++]);
        }
    }
    while (i < (int)sorted1.size()) {
        mergedArray.push_back(sorted1[i++]);
    }
    while (j < (int)sorted2.size()) {
        mergedArray.push_back(sorted2[j++]);
    }
    return mergedArray;
}
