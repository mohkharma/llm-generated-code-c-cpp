...
#include "../../source/output/gpt4o/cpp/Task41_OPENAI_gpt-4o.cpp"  // GPT حل
...
void run_test(const std::string& test_name, const std::string& simulated_input, const std::string& expected_output) {
    ...
    processUserInput();  // الدالة من الحل
...
std::string format_expected_output(const std::string& input) {
    ...
    oss << "Please enter a string: You entered: " << input << "\n"
        << "Length of the input string: " << input.length();
    return oss.str();
}
