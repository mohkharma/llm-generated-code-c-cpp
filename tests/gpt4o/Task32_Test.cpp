...
void testExactThreeYears() {
    ...
    assert(calculateYears(1000.00, 0.05, 0.18, 1100.00) == 3);
}

void testZeroYears() {
    // Test case where desired sum equals initial principal
    assert(calculateYears(1000.00, 0.05, 0.18, 1000.00) == 0);
}

void testSmallIncreaseOneYear() {
    // Test case where it takes exactly 1 year to reach the desired sum
    assert(calculateYears(1000.00, 0.05, 0.18, 1040.00) == 1);
}

void testLongTermInvestment() {
    // Test case where it takes many years to reach the desired sum
    assert(calculateYears(1000.00, 0.05, 0.18, 2000.00) == 18); //todo fix for all languages
}

void testNoTax() {
    // Test case where there is no tax on the interest
    assert(calculateYears(1000.00, 0.05, 0.00, 2000.00) == 15);
}

void testHighTax() {
    // Test case where the tax rate is high
    assert(calculateYears(1000.00, 0.05, 0.50, 2000.00) == 29); //todo fix for all languages
}

void testHighInterestLowTax() {

    // Test case with high interest and low tax rate
    assert(calculateYears(1000.00, 0.10, 0.10, 2000.00) == 9  ); //todo fix for all languages
}

void testLowInterestHighTax() {
    // Test case with low interest and high tax rate
    assert(calculateYears(1000.00, 0.02, 0.50, 2000.00) == 70);//todo fix for all languages
}

void testMinimalInterest() {
    // Test case where the interest rate is very small
    assert(calculateYears(1000.00, 0.005, 0.18, 2000.00) == 170);//todo fix for all languages
}

void testHighPrincipalAndDesiredSum() {
    // Test case where both the principal and desired sum are very large
    assert(calculateYears(1000000.00, 0.05, 0.18, 2000000.00) == 18);
}
